...
- Added two regression tests covering this corruption path: `test_smart_update_semantic_files_promoted_first` (semantic_files must be processed before the directory update) and `test_apply_rejects_poisoned_working_copy` (poisoned working copy must produce the friendly error, not a ParseError). 22/22 passing
