...
- Tray "Show log" now opens the built-in `/log` viewer in a browser: newest entries at the top, 5-second auto-refresh, dark monospace layout with file path / line count / byte size / last-modified timestamp in the sticky header — much better for live tailing than Notepad. Falls back to the original Notepad / xdg-open path if `webbrowser.open` fails
- Switching workspaces now shows a full-screen progress overlay broken into three steps: "Submitting switch request → Loading file list → Checking SVN status", so users no longer assume the switch is done when it is still loading. The three SVN loads (modified / classify / conflicted) are now awaited together with `Promise.allSettled` so the overlay reliably dismisses even if one of the SVN calls fails
- Fixed the semantic-merge queue getting stuck on format-only files: when a file differs only in whitespace / attribute order, the three-way diff yields 0 conflicts / 0 auto-resolved and every sheet shows "no changes"; the previous banner "Merging X (1/1)…" with a trailing ellipsis made users think the program was still working and not safe to click. The banner now reads "Please handle: X (1/N) — click 'Apply merge & save' (top-right) to continue", and a prominent green "No semantic differences" CTA card is rendered above the sheet with a "Confirm no diff & finish this file" button that triggers `svn resolve` and advances the queue with a single click
- Added INFO / WARNING business logs on key API entry points so users can trace what happened from the `/log` viewer: `/api/workspaces/switch|add|remove` log workspace changes, `/api/merge/preview` logs file + from_svn_conflict + auto/conflicts counts, `/api/merge/apply` logs resolutions / total_changes / svn_resolved, `/api/merge/svn-mark-resolved` logs success/failure, and `/api/svn/update` logs entry counts (skip/theirs/mine/semantic) plus exit counts (updated/errors). Error paths log a single WARNING line with file name and the first 200 chars of the error message
- **Critical data-corruption fix** in the semantic-merge queue: after the user finished merging, the directory-wide `svn update` would freeze `<<<<<<<` / `=======` / `>>>>>>>` markers into the working copy. The previous order was `svn update --accept postpone <dir>` followed by `svn resolve --accept working` for each semantic file, but SVN re-merges BASE (old) vs HEAD vs working for those files and re-injects conflict markers; `resolve --accept working` then accepts the working file *including* the markers (this is by SVN design - `--accept working` does not strip markers). The corrupted XML was thus persisted as a "resolved" file. New flow: for each semantic file we run `svn resolve --accept working` + `svn update --accept working <fpath>` *before* the directory update, promoting BASE to HEAD individually so the directory update no longer touches them
- Frontend `applyMerge` now has a reentry guard (`state.mergeApplyInFlight` + button disabled + clears `state.mergeData` / `activeSheet` on success), preventing the user from re-triggering apply on a file whose working copy has already been modified by an in-flight queue advance / svn update
- Backend `_resolve_merge_sources` fallback path now detects whether the working-copy bytes contain any `<<<<<<<` / `=======` / `>>>>>>>` conflict marker line and returns a friendly Chinese error ("working copy contains SVN conflict markers, please clean up manually or `svn revert`") instead of letting ElementTree crash with "not well-formed (invalid token): line 3, column 1". The same guard now also runs on the SVN `.mine` sidecar
- Added two regression tests covering this corruption path: `test_smart_update_semantic_files_promoted_first` (semantic_files must be processed before the directory update) and `test_apply_rejects_poisoned_working_copy` (poisoned working copy must produce the friendly error, not a ParseError)
- Fixed a "local changes selected file -> switch to semantic merge tab -> fake 'no semantic differences' card -> confirm/queue picks up the non-conflict file again" infinite loop: `setMode("merge")` is now `async`, awaits `loadConflictedFiles` and clears `state.selectedFile` if it is not in the SVN conflict set; the "no semantic differences" confirmation banner is now gated on `from_svn_conflict=true` so confirming a non-conflict file (which would be a no-op anyway) is no longer offered
- Added a collapsible status-dot legend at the bottom of the sidebar (red/green/red/orange/grey for SVN conflict / new file / deleted / data change / metadata-only change) so users can self-discover what each dot color means, especially the easily-overlooked grey "metadata only" state
- **SVN external-state drift defense**: prevents silent data desync when the user runs `svn resolve` / `svn update` / saves the file in another editor between SmartDiff's preview cache and the actual disk state. `/api/merge/preview` now returns a `merge_signature` (an `is_conflict` + `theirs_revision` + `mine_mtime` triple), the client echoes it back on `/api/merge/apply`; if the freshly-computed signature differs the backend returns HTTP 409 + `stale: true` and the frontend alerts the user and silently re-runs preview. Covered scenarios: preview-in-conflict → apply-after-resolve, preview-clean → apply-after-update-introduced-conflict, mtime change, and remote HEAD bumped by someone else. Merge mode also gains a 30s automatic poll of the conflict list
- `static/js/app.js` `api()` helper now attaches `err.status` and `err.body` on the error path so callers can differentiate e.g. 409 stale from a generic 500 while remaining compatible with existing `alert(e.message)` callers
- Added four SVN drift regression tests: `test_apply_stale_signature_conflict_to_resolved` (external resolve flips is_conflict), `test_apply_stale_signature_resolved_to_conflict` (external update introduces conflict), `test_apply_stale_signature_mtime_changed` (mtime drift), `test_apply_mine_sidecar_vanished` (vanished .mine sidecar surfaces as a friendly "sidecar file" error, not a ParseError). 26/26 passing
